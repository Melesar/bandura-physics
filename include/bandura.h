...
typedef struct {
  bnd_malloc_fn malloc;   /**< Required allocation callback. */
  bnd_realloc_fn realloc; /**< Optional resize callback; needed to grow configured buffers. */
  bnd_free_fn free;       /**< Required deallocation callback. */
} bnd_allocator;
...
typedef struct {
  bnd_shape_type type; /**< Selects the active member of @c value. */
...
typedef struct {
  ...
  bnd_v3 direction;    /**< Ray direction; callers should provide a unit vector. */
...
/** Initial capacities for world-owned storage. */
typedef struct {
...
typedef struct {
  uint32_t max_age;                   /**< Frames a cached feature may remain unused. */
...
/** Draw one collision contact. @param user_data Value passed to @c bnd_debug_draw. */
typedef void (*bnd_debug_draw_contact_fn)(bnd_v3 point, bnd_v3 normal, float depth, void *user_data);
/** Draw one body shape. @param user_data Value passed to @c bnd_debug_draw. */
...
/** Draw one body axis-aligned bounding box. @param user_data Value passed to @c bnd_debug_draw. */
typedef void (*bnd_debug_draw_aabb_fn)(bnd_v3 center, bnd_v3 size, bnd_body_handle body_handle, void *user_data);
/** Draw one distance joint. @param user_data Value passed to @c bnd_debug_draw. */
...
/** Result types pair an error with a returned value. On failure, @c error is
 * nonzero and @c value is not usable. The macro also declares a constructor
 * named @c bnd_result_<suffix>_error. */
...
/** State used to enumerate bodies without exposing world storage. */
typedef struct {
