...
static void draw_simplex(const simplex *s) {
  ...
  Vector3 p1 = vec_ray(s->points[1].v);
  Vector3 p0 = vec_ray(s->points[0].v);
...
void scenario_setup_scene(bnd_world *world) {
  bnd_body b1 = bnd_add_box_dynamic(world, 1, one());
  bnd_body b2 = bnd_add_box_dynamic(world, 1, one());
  *b1.position = vec3(0, 5, 0);
  *b2.position = vec3(1, 6, 1);
...
void scenario_draw_scene(bnd_world *world) {
  simplex s;
  const common_data *data = as_common_const(world, BND_DYNAMIC);
