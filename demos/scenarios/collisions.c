...
bool collapsed;
count_t pair_index, prev_pair_index;
count_t case_index, prev_case_index;
...
void scenario_setup_scene(bnd_world *world) {
  for (count_t i = 0; i < tests->num_pairs; ++i) {
    for (count_t j = 0; j < tests->cases_per_pair; ++j) {
      ...
      test_results[i * tests->cases_per_pair + j] = bnd_event_any(world, handles[0]).value == tests->cases[i * tests->cases_per_pair + j].intersection;
      bnd_event_unsubscribe_all(world, handles[0]);
    }
  }
...
void scenario_simulate(bnd_world *world, float dt) {
  ...
  bnd_awaken_body(world, handles[0]);
  bnd_awaken_body(world, handles[1]);
...
void scenario_build_ui(bnd_world *world) {
  ...
  ui_value_int("Pair", (int*)&pair_index, 0, tests->num_pairs - 1);
  ui_value_int("Case", (int*)&case_index, 0, tests->cases_per_pair - 1);
              ...
              for (count_t j = 0; j < results_per_row; ++j) {
                ...
                CLAY_AUTO_ID({
                  ...
                  if (is_hovering && pointer_state == CLAY_POINTER_DATA_RELEASED_THIS_FRAME) {
                    case_index = result_index - pair_results_start;
                    update_pair(world);
                  }
                }
              }
