...
static void test_raycast_multiple_respects_limits_and_body_types(void) {
  ...
  assert(bnd_raycast_multiple(world, ray, ~0, hits, 0) == 0);
  assert(bnd_raycast_multiple(world, ray, ~0, hits, 1) == 1);
  expect_handle(hits[0].body, dynamic);
  assert(bnd_raycast_multiple(world, ray, ~0, hits, 2) == 2);
  expect_handle(hits[0].body, dynamic);
  expect_handle(hits[1].body, static_body);

  ray.max_distance = 1.0f;
  assert(bnd_raycast_multiple(world, ray, ~0, hits, 2) == 0);
  bnd_teardown(world);
}
...
static void test_overlap_covers_dynamic_and_static_bodies_with_distinct_capacities(void) {
  ...
  assert(bnd_overlap(world, bnd_v3_zero(), 1.0f, ~0, overlaps, 0) == 0);
  assert(bnd_overlap(world, bnd_v3_zero(), 1.0f, ~0, overlaps, 1) == 1);
  expect_handle(overlaps[0], dynamic);
  assert(bnd_overlap(world, bnd_v3_zero(), 1.0f, ~0, overlaps, 2) == 2);
  expect_handle(overlaps[0], dynamic);
  expect_handle(overlaps[1], static_body);

  assert(bnd_overlap(world, (bnd_v3){10, 0, 0}, 0.5f, ~0, overlaps, 2) == 0);
  bnd_teardown(world);
}
